		...
		if ( $user_exists ) {
			...
			if ( $model->save() ) {
				...
				try {
					...
					#print_r($user);exit;
					Session::flash( 'message', 'Successfully sent ' . $email . ' to reset password. Please check your inbox or spam!' );
				} catch ( \Exception $e ) {
	...
	public function user_info( $value ) {
		...
		try {
			...
			if ( $value == 'acc-settings' ) {
				...
				return Response::json( view( 'user::user_info.account_settings._ajax_data', [
					'user_data'    => $user_data,
					'profile_data' => $profile_data
				] )->render() );
			}
...
//                unlink($model->thumbnail);
	...
	public function edit_user_profile( $id ) {
		...
		return view( 'user::user_info.profile.update', [
			'pageTitle'  => $pageTitle,
			'data'       => $data,
			'user_id'    => $user_id,
			'user_image' => $user_image
...
//                unlink($model->thumbnail);
	...
	public function edit_profile_image( $user_image_id ) {
		...
		return view( 'user::user_info.profile_image.update_image', [
			'pageTitle'     => $pageTitle,
			'model'         => $model,
...
//                unlink($model->thumbnail);
	...
	public function edit_meta_data( $id ) {
		...
		return view( 'user::user_info.meta_data.update', [
			'pageTitle' => $pageTitle,
			'data'      => $data,
			'user_id'   => $user_id
		] );
