...
class RoleUserController extends Controller
{
    ...
    public function store(Requests\RoleUserRequest $request){
        ...
        if(!$role_user_exists){
            ...
            try {
                ...
                DB::rollback();
                Session::flash('danger-info', $e->getMessage());
                #LogFileHelper::log_error('store-role-user', $e->getMessage(), ['Role user (user_id): '.$input['user_id']]);
            }

        }else{
            Session::flash('danger-info','User role already exists.');
        }
    ...
    public function update(Requests\RoleUserRequest $request, $id)
    {
        ...
        $input = $request->all();
        $role_user_exists = RoleUser::where('id','!=',$id)-> where('user_id',$input['user_id'])->where('role_id',$input['role_id'])->exists();
        if(!$role_user_exists){
            DB::beginTransaction();
            try {
                $model->update($input);
