    ...
    <section
        ...
        <div class="panel-heading">
            ...
            <a class="btn-sm btn-success pull-right paste-blue-button-bg"
               data-toggle="modal"
               href="#addData">
                <b>Add Umrah Client Information</b>
            </a>
                    ...
                    @if(isset($data))
                        @foreach($data as $values)
                            <tr class="gradeX">
                                ...
                                <td>{{ isset($values->umrah_fair) ? ucfirst($values->umrah_fair) : 'N/A' }}</td>
                                <td>{{ isset($values->relUmrahClientPaymentReceive) ? $values->relUmrahClientPaymentReceive->sum('receive_amount') : 'N/A' }}</td>
                                <?php
                                if (isset($values->umrah_fair)) {
                                    $dueAmount = $values->umrah_fair - $values->relUmrahClientPaymentReceive->sum('receive_amount');
                                }
                                ?>
                                <td>{{ $dueAmount or 'N/A' }}</td>
                                <td>{{ isset($values->status) ? ucfirst($values->status) : 'N/A' }}</td>
                                <td>
                                    <a href="{{ route('view-umrah-client-info', $values->id) }}"
                                    ...
                                    <a href="{{ route('edit-umrah-client-info', $values->id) }}"
                                    ...
                                    <a href="{{ route('delete-umrah-client-info', $values->id) }}"
                                    ...
                                    <a href="{{ route('receive-umrah-client-info', $values->id) }}"
                                       class="ddd btn btn-primary btn-xs"
                                       data-toggle="modal"
                                       data-target="#etsbModal"
                                       data-placement="top"
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Add Umrah Client Information</h4>
                </div>
                <div class="modal-body">
                    {!! Form::open(['route' => 'store-umrah-client-info','id' => 'umrah-client-info-add-form']) !!}
                    @include('umrah::umrah-client-info._form')
                    {!! Form::close() !!}
                </div>
                <!-- / .modal-body -->
...
@section('custom-script')
    <script>
        ...
        $(function () {
            $('#umrah-client-info-add-form').validate({
                rules: {
                    client_name: {
                        required: true,
