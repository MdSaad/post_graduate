    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            <table id=""
                ...
                <tr>
                    <th class="col-lg-4">Umrah Fair</th>
                    <td>{{ isset($data)?ucfirst($data->umrah_fair):''}}</td>
                </tr>
                <tr>
                    <th class="col-lg-4">Receive Amount</th>
                    <td>{{ isset($data)?$data->relUmrahClientPaymentReceive->sum('receive_amount'):''}}</td>
                </tr>
                <tr>
                    <th class="col-lg-4">Due Amount</th>
                    <?php
                    if (isset($data->umrah_fair)) {
                        $dueAmount = $data->umrah_fair - $data->relUmrahClientPaymentReceive->sum('receive_amount');
                    }
                    ?>
                    <td>{{ $dueAmount or ''  }}</td>
                </tr>
